...
x<-cbind(PC1_3,metadata)
#By EcoRegion
ggplot(data=pop_K3.combined.merged_metadata, aes(y=V1, x=V2))+geom_point(size=7,color="black",aes(shape=SPECIES,fill=HABITAT))+ theme_classic() + xlab("PC2 (2.77%)") +ylab("PC1 (3.60%)")+geom_hline(yintercept=0,linetype="dashed")+geom_vline(xintercept =0,linetype="dashed")+scale_fill_manual("Ecoregion", values =c("EOR"="red","SSOPR"="bisque","MGPR"="blue","SSBPR"="darkorchid1","SHGPR"="darkolivegreen3"))+scale_shape_manual("Species", values=c(25,21))+ theme(legend.position = "top")
