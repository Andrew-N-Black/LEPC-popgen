...
1 Outside      9 0.0456 0.0504 0.0218 0.0121
2 Northern   274 0.0504 0.0623 0.0272 0.0822
3 Southern   150 0.0991 0.0457 0.0862 0.0438
...
Southern 0.0035  <2e-16  
  ...
  DPS      count   mean     sd  median      IQR
  <fct>    <int>  <dbl>  <dbl>   <dbl>    <dbl>
1 Outside      9 0.0109 0.0181 0.00232 0.000977
2 Northern   274 0.0116 0.0206 0.00166 0.0153  
3 Southern   150 0.0203 0.0165 0.0165  0.0172  
...
+     summarise(count = n(), mean = mean(`fROH_100kb-1Mb`, na.rm = TRUE), sd = sd(`fROH_100kb-1Mb`, na.rm = TRUE),median = median(`fROH_100kb-1Mb`, na.rm = TRUE),IQR = IQR(`fROH_100kb-1Mb`, na.rm = TRUE))
 ...
 DPS      count   mean     sd median    IQR
  <fct>    <int>  <dbl>  <dbl>  <dbl>  <dbl>
1 Outside      9 0.0347 0.0324 0.0196 0.0117
2 Northern   274 0.0388 0.0429 0.0256 0.0691
3 Southern   150 0.0788 0.0301 0.0710 0.0269

pairwise.wilcox.test(HET_FILT$`fROH_100kb-1Mb`, HET_FILT$DPS, p.adjust.method = "BH")

data:  HET_FILT$`fROH_100kb-1Mb` and HET_FILT$DPS 

         Outside Northern
Northern 0.903   -       
Southern 0.002   <2e-16  
