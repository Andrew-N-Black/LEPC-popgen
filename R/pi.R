...
#Fst only
library(readxl)
fst_plot <- read_excel("fst_plot.xlsx")
View(fst_plot)
ggplot(fst_plot,aes(x=chr_mid, y=fst,color=chr,fill=outlier)) + geom_line(linewidth=0.09,group=1)+theme(axis.text.x=element_text(size=.01, angle=90))+theme_classic()+theme(axis.title.x=element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank())+ylab("π")+scale_color_manual(values=rep(c("black","grey"),98))+theme(legend.position = "none")+geom_hline(yintercept=0.14, linetype="dashed",color = "blue", size=0.5)+ylim(c(0,1))+theme(legend.position = "none")+geom_hline(yintercept=0.736254, linetype="solid",color = "blue", size=0.5)
#Pi only
